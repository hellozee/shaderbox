...
GLuint Shader::get() { return m_shader; }
...
Program::Program(const std::string &vertex_source,
                 const std::string &fragment_source) {
  m_program = glCreateProgram();

  m_vertex_shader =
      Shader{load_from_file(vertex_source), GL_VERTEX_SHADER, m_program}.get();
  m_fragment_shader =
      Shader{load_from_file(fragment_source), GL_FRAGMENT_SHADER, m_program}
          .get();
...
GLuint Program::get() { return m_program; }
